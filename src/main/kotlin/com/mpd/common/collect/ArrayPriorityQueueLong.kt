...
//
// Not thread-safe: this is a fork of ArrayPriorityBlockingQueueLong with the locking removed.
class ArrayPriorityQueueLong(
    ...
    fun clone() : ArrayPriorityQueueLong {
        val c = ArrayPriorityQueueLong(size, comparator)
    ...
    override fun equals(other: Any?) : Boolean {
        if (other !is ArrayPriorityQueueLong) return false
    ...
    override fun toString() : String = "LongPriorityArrayQueue[size=$size top=${if(size>0)queue[0] else "null"}]"

    fun forEach(action: (Long) -> Unit): Unit {
