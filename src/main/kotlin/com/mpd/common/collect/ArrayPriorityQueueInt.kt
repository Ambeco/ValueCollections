...
//
// Not thread-safe: this is a fork of ArrayPriorityBlockingQueueInt with the locking removed.
class ArrayPriorityQueueInt(
    ...
    fun containsAll(c: IntArray): Boolean {
        for (item in c) {
            if (!contains(item)) return false
        }
        return true
    ...
    fun addAll(c: Collection<Int>) : Boolean {
        ++mutCounter
        val oldSize = size
    ...
    fun removeAll(c: IntArray) : Boolean {
        ++mutCounter
        if (c.size > size) {
            return removeIf { c.contains(it) }
        }
    ...
    fun clone() : ArrayPriorityQueueInt {
        val c = ArrayPriorityQueueInt(size, comparator)
    ...
    override fun equals(other: Any?) : Boolean {
        if (other !is ArrayPriorityQueueInt) return false
    ...
    override fun toString() : String = "IntPriorityArrayQueue[size=$size top=${if(size>0)queue[0] else "null"}]"
    ...
    inner class Spliterator(var index: Int, var endIndex: Int, val mutSnapshot: Int) : java.util.Spliterator.OfInt {
        ...
        override fun characteristics(): Int {
            return SIZED.or(NONNULL).or(IMMUTABLE).or(SUBSIZED)
        }
    }
