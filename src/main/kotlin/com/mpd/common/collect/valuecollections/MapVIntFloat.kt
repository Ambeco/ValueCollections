...
class MutMapVIntFloat<K>(override val collection: MutableIntFloatMap = MutableIntFloatMap()): MapVIntFloat<K> {
    ...
    context(a: ValueIntAdapter<K>) inline infix operator fun minusAssign(key: K):Unit = collection.minusAssign(a.toInt(key))
    inline infix operator fun minusAssign(keys: IntArray):Unit = collection.minusAssign(keys) // TODO: Add VIntArray/VIntCollection
    inline infix operator fun minusAssign(keys: IntSet):Unit = collection.minusAssign(keys)
    inline infix operator fun minusAssign(keys: IntList):Unit = collection.minusAssign(keys) 
