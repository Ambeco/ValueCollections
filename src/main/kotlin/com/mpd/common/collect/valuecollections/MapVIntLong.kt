...
interface MapVIntLong<K,V>: MapVIntKey<K>, MapVLongValue<V> {
    ...
    context(ka: ValueIntAdapter<K>, va: ValueLongAdapter<V>) fun toIterable(): Iterable<PairVIntLong<K,V>>
...
context(ka: ValueIntAdapter<K>, va: ValueLongAdapter<V>) inline fun <K,V, A : Appendable> MapVIntLong<K,V>.joinTo(buffer: A, separator: CharSequence = ", ", prefix: CharSequence = "", postfix: CharSequence = "", limit: Int = size, truncated: CharSequence = "...", crossinline transform: ((K, V) -> CharSequence) = { k, v-> "($k:$v)" }): A {
...
context(ka: ValueIntAdapter<K>, va: ValueLongAdapter<V>) inline fun <K,V> MapVIntLong<K,V>.joinToString(separator: CharSequence = ", ", prefix: CharSequence = "", postfix: CharSequence = "", limit: Int = size, truncated: CharSequence = "...", crossinline transform: ((K, V) -> CharSequence) = { k, v-> "($k:$v)" }): String
...
interface MutableMapVIntLong<K,V>: MapVIntLong<K,V> {
    ...
    context(ka: ValueIntAdapter<K>, va: ValueLongAdapter<V>) override fun toIterable(): Iterable<PairVIntLong<K,V>>
...
inline infix operator fun <K,V> MutableMapVIntLong<K,V>.plusAssign(source: MapVIntLong<K,V>) = putAll(source)
...
context(ka: ValueIntAdapter<K>) inline infix operator fun <K,V> MutableMapVIntLong<K,V>.minusAssign(key: K) {remove(key)}
...
class HashMapVIntLong<K,V>(val collection: MutableIntLongMap=MutableIntLongMap(), override val NULL_KEY_BITS: IntKeyBits=Int.MIN_VALUE, override val NULL_VALUE_BITS: LongValueBits=Long.MIN_VALUE)
    ...
    override inline fun getBits(k: IntKeyBits): LongValueBits = collection.getOrDefault(k, NULL_VALUE_BITS)
    override inline fun anyBits(predicate: (IntKeyBits, LongValueBits) -> Boolean): IntKeyBits {
        val finder = object : (IntKeyBits, LongValueBits) -> Unit {
            var result: IntKeyBits = NULL_KEY_BITS
            override inline fun invoke(k: IntKeyBits, v: LongValueBits) { if (result == NULL_KEY_BITS && predicate(k, v)) result = k }
        }
    ...
    // Thin wrappers for every public method of MutableIntLongMap.
    ...
    inline fun getOrDefault(key: IntKeyBits, defaultValue: LongValueBits): LongValueBits = collection.getOrDefault(key, defaultValue)
    inline fun getOrElse(key: IntKeyBits, defaultValue: () -> LongValueBits): LongValueBits = collection.getOrElse(key, defaultValue)
    inline fun forEachBits(block: (key: IntKeyBits, value: LongValueBits) -> Unit) = collection.forEach(block)
    ...
    inline fun all(predicate: (IntKeyBits, LongValueBits) -> Boolean): Boolean = collection.all(predicate)
    inline fun count(): Int = collection.count()
    inline fun count(predicate: (IntKeyBits, LongValueBits) -> Boolean): Int = collection.count(predicate)
    ...
    inline fun joinToStringBits(
        ...
        limit: Int = -1,
        truncated: CharSequence = "...",
        crossinline transform: (key: IntKeyBits, value: LongValueBits) -> CharSequence,
    ): String = collection.joinToString(separator, prefix, postfix, limit, truncated, transform)
    inline fun setBits(key: IntKeyBits, value: LongValueBits) = collection.set(key, value)
    inline fun put(key: IntKeyBits, value: LongValueBits) = collection.put(key, value)
    inline fun put(key: IntKeyBits, value: LongValueBits, default: LongValueBits): LongValueBits = collection.put(key, value, default)
    inline fun putAllBits(from: IntLongMap) = collection.putAll(from)
    inline fun plusAssignBits(from: IntLongMap) = collection.plusAssign(from)
    ...
    context(ka: ValueIntAdapter<K>, va: ValueLongAdapter<V>) override inline fun toIterable(): Iterable<PairVIntLong<K,V>> {
        val list = ArrayList<PairVIntLong<K,V>>(size)
        collection.forEach { k, v -> list.add(PairVIntLong(k, v)) }
        return object : Iterable<PairVIntLong<K,V>> {
            override inline fun iterator(): Iterator<PairVIntLong<K,V>> = object : Iterator<PairVIntLong<K,V>> {
                var idx = 0
                override inline fun hasNext(): Boolean = idx < list.size
                override inline fun next(): PairVIntLong<K,V> { val p = list[idx++]; return p }
            }
        }
    }
