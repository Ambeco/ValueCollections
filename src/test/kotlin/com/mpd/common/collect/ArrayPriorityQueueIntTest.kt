...
private fun drain(q: ArrayPriorityQueueInt): List<Int> {
    val result = mutableListOf<Int>()
    while (q.isNotEmpty()) result.add(q.poll())
    return result
}

class ArrayPriorityQueueIntTest {
    @Test
    fun constructorsAndBasics() {
        val default = ArrayPriorityQueueInt()
        assertEquals(0, default.size())
        assertEquals(true, default.isEmpty())
        assertEquals(false, default.isNotEmpty())

        val byCapacity = ArrayPriorityQueueInt(5)
        assertEquals(0, byCapacity.size())

        val fromCollection = ArrayPriorityQueueInt(listOf(3, 1, 2))
        assertEquals(3, fromCollection.size())
        assertEquals(1, fromCollection.peek())

        val fromArray = ArrayPriorityQueueInt(intArrayOf(3, 1, 2))
        assertEquals(3, fromArray.size())
        assertEquals(1, fromArray.peek())

        val maxHeap = ArrayPriorityQueueInt(comparator = reverseComparator)
        maxHeap.add(1)
        maxHeap.add(3)
        maxHeap.add(2)
    ...
    @Test
    fun clear() {
        val q = ArrayPriorityQueueInt(listOf(1, 2, 3))
        q.clear()
    ...
    fun addAllVariantsReturnValueAndOrder() {
        val byCollection = ArrayPriorityQueueInt()
        ...
        val byArray = ArrayPriorityQueueInt()
    ...
    fun pollOrdersBySmallestFirstAndThrowsWhenEmpty() {
        val q = ArrayPriorityQueueInt(listOf(9, 4, 7, 1, 3))
    ...
    fun removeAndMinusRemoveSpecificValue() {
        ...
        val q2 = ArrayPriorityQueueInt(listOf(1, 2, 3))
        q2 - 1
    ...
    @Test
    fun removeIf() {
        val q = ArrayPriorityQueueInt(listOf(1, 2, 3, 4, 5))
    ...
    fun retainAllVariants() {
        val byCollection = ArrayPriorityQueueInt(listOf(1, 2, 3, 4))
        assertEquals(true, byCollection.retainAll(listOf(2, 4)))
        assertEquals(listOf(2, 4), drain(byCollection)
        )

        val byArray = ArrayPriorityQueueInt(listOf(1, 2, 3, 4))
    ...
    fun cloneIsIndependentAndEqual() {
        val original = ArrayPriorityQueueInt(100,
            reverseComparator
        )
    ...
    fun equalsConsidersComparatorAndContent() {
        val a = ArrayPriorityQueueInt(listOf(1, 2, 3))
        val b = ArrayPriorityQueueInt(listOf(1, 2, 3))
        assertEquals(true, a == b)
        assertEquals(a.hashCode(), b.hashCode())

        val differentComparator = ArrayPriorityQueueInt(listOf(1, 2, 3),
            reverseComparator
        )
        assertEquals(false, a == differentComparator)

        val differentContent = ArrayPriorityQueueInt(listOf(1, 2, 99))
    ...
    fun toStringContainsSizeAndTop() {
        val q = ArrayPriorityQueueInt(listOf(5, 3, 8))
        val s = q.toString()
    ...
    fun forEachVisitsAllElements() {
        val q = ArrayPriorityQueueInt(listOf(1, 2, 3, 4))
        var sum = 0
        q.forEach { sum += it }
        assertEquals(10, sum)
    }
    ...
    fun spliteratorTryAdvanceAndSplitAndCharacteristics() {
        ...
        val tiny = ArrayPriorityQueueInt(listOf(1)).spliterator()
    ...
    fun streamAndParallelStream() {
        ...
        assertEquals(10, ArrayPriorityQueueInt(listOf(1, 2, 3, 4)).parallelStream().sum())
    }
}
