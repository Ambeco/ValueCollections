...
class MutableIndexedCollectionVIntTest {
    ...
    fun addAtIndex() = with (MutIdxColTestClass) {
        val list = ArrayListVInt<MutIdxColTestClass>().also { it plusAssign MutIdxColTestClass(1); it += MutIdxColTestClass(3) }
        list.addBits(1, 2)
        assertEquals(
            vIntListOf(
                MutIdxColTestClass(1),
                MutIdxColTestClass(2),
                MutIdxColTestClass(3)
            ), list)
        list.add(0, MutIdxColTestClass(0))
        assertEquals(
            vIntListOf(
                MutIdxColTestClass(0),
                MutIdxColTestClass(1),
                MutIdxColTestClass(2),
                MutIdxColTestClass(3)
            ), list)
    ...
    fun addAllAtIndexNotYetImplemented() = with (MutIdxColTestClass) {
        ...
        assertThrows(NotImplementedError::class.java, { list.addAll(0,
            vIntListOf(MutIdxColTestClass(2))
        ) })
