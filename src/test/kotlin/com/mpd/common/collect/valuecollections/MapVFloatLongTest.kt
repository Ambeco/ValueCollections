...
import com.mpd.common.collect.valuecollections.HashMapVFloatLong
...
@JvmInline
value class MFLValClass(val value: Long): Comparable<MFLValClass> {
    override operator fun compareTo(other: MFLValClass): Int = value.compareTo(other.value)
    override fun toString(): String = value.toString()
    @Suppress("OVERRIDE_BY_INLINE", "NOTHING_TO_INLINE")
    companion object PrimitiveLongAdapter: ValueLongAdapter<MFLValClass> {
        override inline fun fromLong(v: Long) = MFLValClass(v)
        override inline fun toLong(v: MFLValClass): Long = v.value
    }
}

class MapVFloatLongTest {
    private fun simpleMap(): HashMapVFloatLong<MFLValClass> = with (MFLValClass) {
        HashMapVFloatLong<MFLValClass>().also {
            it[1f] = MFLValClass(1)
            it[2f] = MFLValClass(2)
            it[3f] = MFLValClass(3)
        }
    ...
    @Test
    fun constructors() {
        val primary = HashMapVFloatLong<MFLValClass>()
        assertEquals(0, primary.size)
        val bySize = HashMapVFloatLong<MFLValClass>(10)
        assertEquals(0, bySize.size)
    }

    @Test
    fun customNullValue() {
        val map = HashMapVFloatLong<MFLValClass>(10, -1)
        assertEquals(-1, map.NULL_VALUE_BITS)
    }

    @Test
    fun getSetBits() {
        val map = HashMapVFloatLong<MFLValClass>()
        assertEquals(map.NULL_VALUE_BITS, map.setBits(1f, 1, map.NULL_VALUE_BITS))
        assertEquals(1L, map.getBits(1f))
        assertEquals(1L, map.setBits(1f, 11, map.NULL_VALUE_BITS))
        assertEquals(11L, map.getBits(1f))
    ...
    fun getOrPutBits() {
        val map = HashMapVFloatLong<MFLValClass>()
        assertEquals(1L, map.getOrPutBits(1f) { 1 })
        assertEquals(1L, map.getOrPutBits(1f) { 999 })
    }
    ...
    @Test
    fun typedGetVariants() = with (MFLValClass) {
        val map = simpleMap()
        assertEquals(MFLValClass(2), map[2f])
        assertEquals(MFLValClass(2), map.getOr(2f) { MFLValClass(-1) })
        assertEquals(MFLValClass(-1), map.getOr(99f) { MFLValClass(-1) })
        assertEquals(MFLValClass(2), map.getOrNull(2f))
        assertEquals(null, map.getOrNull(99f))
    }

    @Test
    fun anyVariants() = with (MFLValClass) {
        val map = simpleMap()
        assertEquals(2f, map.any { _, v -> v.value == 2L })
        assertEquals(2f, map.anyOr({ _, v -> v.value == 2L }) { -1f })
        assertEquals(-1f, map.anyOr({ _, v -> v.value == 999L }) { -1f })
    }

    @Test
    fun anyIndexedVariants() = with (MFLValClass) {
        val map = simpleMap()
        val visitedIndices = mutableSetOf<Int>()
        map.forEachIndexed { i, _, _ -> visitedIndices.add(i) }
        assertEquals(setOf(0, 1, 2), visitedIndices)
        assertEquals(2f, map.anyIndexed { _, k, v -> v.value == 2L })
        assertEquals(2f, map.anyIndexedOr({ _, k, v -> v.value == 2L }) { -1f })
        assertEquals(-1f, map.anyIndexedOr({ _, _, v -> v.value == 999L }) { -1f })
    }

    @Test
    fun forEachVariants() = with (MFLValClass) {
        val map = simpleMap()
        var sum = 0L
    ...
    @Test
    fun containsKeyAndValue() = with (MFLValClass) {
        val map = simpleMap()
        assertEquals(true, map.containsKey(2f))
        assertEquals(false, map.containsKey(99f))
        assertEquals(true, map.containsValue(MFLValClass(2)))
        assertEquals(false, map.containsValue(MFLValClass(999)))
    }

    @Test
    fun toStringVSingleEntry() = with (MFLValClass) {
        val map = HashMapVFloatLong<MFLValClass>().also { it[1f] = MFLValClass(1) }
        assertEquals("{(1.0:1)}", map.toStringV())
    }

    @Test
    fun joinToStringLimit() = with (MFLValClass) {
        val map = simpleMap()
    ...
    @Test
    fun setOperatorAndWithDefault() = with (MFLValClass) {
        val map = HashMapVFloatLong<MFLValClass>()
        assertEquals(false, map.set(1f, MFLValClass(1)))
        assertEquals(true, map.set(1f, MFLValClass(11)))
        assertEquals(MFLValClass(11), map[1f])

        assertEquals(MFLValClass(-1), map.set(99f, MFLValClass(50), MFLValClass(-1)))
        assertEquals(MFLValClass(50), map.set(99f, MFLValClass(60), MFLValClass(-1)))
    }

    @Test
    fun getOrPut() = with (MFLValClass) {
        val map = HashMapVFloatLong<MFLValClass>()
        assertEquals(MFLValClass(1), map.getOrPut(1f) { MFLValClass(1) })
        assertEquals(MFLValClass(1), map.getOrPut(1f) { MFLValClass(999) })
    }

    @Test
    fun putAllAndPlusAssign() = with (MFLValClass) {
        val map = HashMapVFloatLong<MFLValClass>()
        map.putAll(simpleMap())
        assertEquals(3, map.size)

        val map2 = HashMapVFloatLong<MFLValClass>()
        map2 plusAssign simpleMap()
        assertEquals(3, map2.size)
    }

    @Test
    fun putAllGeneric() = with (MFLValClass) {
        val map = HashMapVFloatLong<MFLValClass>()
        map.putAllGeneric(listOf(1f to 1L, 2f to 2L)) { (k, v) -> k to MFLValClass(v) }
        assertEquals(2, map.size)
        assertEquals(MFLValClass(2), map[2f])
    }

    @Test
    fun removeVariants() = with (MFLValClass) {
        val map = simpleMap()
        map.remove(1f)
        assertEquals(2, map.size)
        assertEquals(true, map.remove(2f, MFLValClass(2)))
        assertEquals(1, map.size)
    ...
    @Test
    fun minusAssign() = with (MFLValClass) {
        val map = simpleMap()
        map minusAssign 1f
        assertEquals(2, map.size)
    }
    ...
    @Test
    fun asMapGenericNonIterating() = with (MFLValClass) {
        val map = simpleMap()
        ...
        assertEquals(true, generic.containsValue(MFLValClass(2)))
        assertEquals(MFLValClass(2), generic[2f])
    ...
    fun rawWrapperMethods() {
        ...
        assertEquals(-1L, map.getOrDefault(99f, -1))
        assertEquals(2L, map.getOrElse(2f) { -1 })
        assertEquals(-1L, map.getOrElse(99f) { -1 })
        ...
        assertEquals(setOf(1L, 2L, 3L), values)
        ...
        map.setBits(4f, 4)
        assertEquals(4L, map.getBits(4f))
        map.put(4f, 5)
        assertEquals(5L, map.getBits(4f))
        assertEquals(5L, map.put(4f, 6, -1))
        assertEquals(-1L, map.put(5f, 5, -1))

        val other = MutableFloatLongMap().also { it[10f] = 1000 }
        map.putAllBits(other)
        assertEquals(1000L, map.getBits(10f))

        val other2 = MutableFloatLongMap().also { it[11f] = 1100 }
        map.plusAssignBits(other2)
        assertEquals(1100L, map.getBits(11f))
