...
private val reverseComparator = object : PriorityBlockingArrayQueueInt.Companion.Comparator {
    override fun invoke(a: Int, b: Int): Int = b.compareTo(a)
}

private fun drain(q: PriorityBlockingArrayQueueInt): List<Int> {
    val result = mutableListOf<Int>()
    while (q.isNotEmpty()) result.add(q.poll())
    return result
}

class PriorityBlockingArrayQueueIntTest {
    @Test
    fun constructorsAndBasics() {
        val default = PriorityBlockingArrayQueueInt()
        assertEquals(0, default.size())
        assertEquals(true, default.isEmpty())
        assertEquals(false, default.isNotEmpty())

        val byCapacity = PriorityBlockingArrayQueueInt(5)
        assertEquals(0, byCapacity.size())

        val fromCollection = PriorityBlockingArrayQueueInt(listOf(3, 1, 2))
        assertEquals(3, fromCollection.size())
        assertEquals(1, fromCollection.peek())

        val fromArray = PriorityBlockingArrayQueueInt(intArrayOf(3, 1, 2))
        assertEquals(3, fromArray.size())
        assertEquals(1, fromArray.peek())

        val maxHeap = PriorityBlockingArrayQueueInt(comparator = reverseComparator)
        maxHeap.add(1)
        maxHeap.add(3)
        maxHeap.add(2)
    ...
    @Test
    fun clear() {
        val q = PriorityBlockingArrayQueueInt(listOf(1, 2, 3))
        q.clear()
    ...
    fun elementAndPeekAndGet() {
        val q = PriorityBlockingArrayQueueInt(listOf(3, 1, 2))
        assertEquals(1, q.element())
        assertEquals(1, q.peek())
        assertEquals(1, q[0])
        assertThrows(IndexOutOfBoundsException::class.java) { q[99] }

        val empty = PriorityBlockingArrayQueueInt()
    ...
    fun addPlusOfferAndHeapOrder() {
        val q = PriorityBlockingArrayQueueInt()
        q.add(5)
        q + 3
        q.offer(8)
        q.add(1)
    ...
    fun addAllVariantsReturnValueAndOrder() {
        val byCollection = PriorityBlockingArrayQueueInt()
        ...
        val byArray = PriorityBlockingArrayQueueInt()
    ...
    fun pollOrdersBySmallestFirstAndThrowsWhenEmpty() {
        val q = PriorityBlockingArrayQueueInt(listOf(9, 4, 7, 1, 3))
    ...
    fun removeAndMinusRemoveSpecificValue() {
        ...
        val q2 = PriorityBlockingArrayQueueInt(listOf(1, 2, 3))
        q2 - 1
    ...
    @Test
    fun removeIf() {
        val q = PriorityBlockingArrayQueueInt(listOf(1, 2, 3, 4, 5))
    ...
    fun retainAllVariants() {
        val byCollection = PriorityBlockingArrayQueueInt(listOf(1, 2, 3, 4))
        assertEquals(true, byCollection.retainAll(listOf(2, 4)))
        assertEquals(listOf(2, 4), drain(byCollection))

        val byArray = PriorityBlockingArrayQueueInt(listOf(1, 2, 3, 4))
    ...
    fun cloneIsIndependentAndEqual() {
        val original = PriorityBlockingArrayQueueInt(100, reverseComparator)
    ...
    fun equalsConsidersComparatorAndContent() {
        val a = PriorityBlockingArrayQueueInt(listOf(1, 2, 3))
        val b = PriorityBlockingArrayQueueInt(listOf(1, 2, 3))
        assertEquals(true, a == b)
        assertEquals(a.hashCode(), b.hashCode())

        val differentComparator = PriorityBlockingArrayQueueInt(listOf(1, 2, 3), reverseComparator)
        assertEquals(false, a == differentComparator)

        val differentContent = PriorityBlockingArrayQueueInt(listOf(1, 2, 99))
    ...
    fun toStringContainsSizeAndTop() {
        val q = PriorityBlockingArrayQueueInt(listOf(5, 3, 8))
        val s = q.toString()
    ...
    fun forEachVisitsAllElements() {
        val q = PriorityBlockingArrayQueueInt(listOf(1, 2, 3, 4))
        var sum = 0
        q.forEach { sum += it }
        assertEquals(10, sum)
    }
    ...
    fun spliteratorTryAdvanceAndSplitAndCharacteristics() {
        ...
        val tiny = PriorityBlockingArrayQueueInt(listOf(1)).spliterator()
    ...
    fun streamAndParallelStream() {
        ...
        assertEquals(10, PriorityBlockingArrayQueueInt(listOf(1, 2, 3, 4)).parallelStream().sum())
    }
}
