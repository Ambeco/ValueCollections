...
private val reverseComparator = object : PriorityBlockingArrayQueueLong.Companion.Comparator {
    override fun invoke(a: Long, b: Long): Int = b.compareTo(a)
}

private fun drain(q: PriorityBlockingArrayQueueLong): List<Long> {
    val result = mutableListOf<Long>()
    while (q.isNotEmpty()) result.add(q.poll())
    return result
}

class PriorityBlockingArrayQueueLongTest {
    @Test
    fun constructorsAndBasics() {
        val default = PriorityBlockingArrayQueueLong()
        assertEquals(0, default.size())
        assertEquals(true, default.isEmpty())
        assertEquals(false, default.isNotEmpty())

        val byCapacity = PriorityBlockingArrayQueueLong(5)
        assertEquals(0, byCapacity.size())

        val fromCollection = PriorityBlockingArrayQueueLong(listOf(3L, 1L, 2L))
        assertEquals(3, fromCollection.size())
        assertEquals(1L, fromCollection.peek())

        val fromArray = PriorityBlockingArrayQueueLong(longArrayOf(3, 1, 2))
        assertEquals(3, fromArray.size())
        assertEquals(1L, fromArray.peek())

        val maxHeap = PriorityBlockingArrayQueueLong(comparator = reverseComparator)
        maxHeap.add(1)
        maxHeap.add(3)
        maxHeap.add(2)
    ...
    @Test
    fun clear() {
        val q = PriorityBlockingArrayQueueLong(listOf(1L, 2L, 3L))
        q.clear()
    ...
    fun addPlusOfferAndHeapOrder() {
        val q = PriorityBlockingArrayQueueLong()
        q.add(5)
        q + 3L
        q.offer(8)
        q.add(1)
    ...
    fun addAllVariantsReturnValueAndOrder() {
        val byCollection = PriorityBlockingArrayQueueLong()
        ...
        val byArray = PriorityBlockingArrayQueueLong()
    ...
    fun pollOrdersBySmallestFirstAndThrowsWhenEmpty() {
        val q = PriorityBlockingArrayQueueLong(listOf(9L, 4L, 7L, 1L, 3L))
    ...
    fun removeAndMinusRemoveSpecificValue() {
        ...
        val q2 = PriorityBlockingArrayQueueLong(listOf(1L, 2L, 3L))
        q2 - 1L
    ...
    fun removeAllVariants() {
        // c.size <= size branch
        val small = PriorityBlockingArrayQueueLong(listOf(1L, 2L, 3L, 4L))
        ...
        val big = PriorityBlockingArrayQueueLong(listOf(1L, 2L))
        assertEquals(true, big.removeAll(listOf(1L, 2L, 99L)))
        assertEquals(0, big.size())

        val byArraySmall = PriorityBlockingArrayQueueLong(listOf(1L, 2L, 3L, 4L))
    ...
    @Test
    fun removeIf() {
        val q = PriorityBlockingArrayQueueLong(listOf(1L, 2L, 3L, 4L, 5L))
    ...
    fun retainAllVariants() {
        val byCollection = PriorityBlockingArrayQueueLong(listOf(1L, 2L, 3L, 4L))
        assertEquals(true, byCollection.retainAll(listOf(2L, 4L)))
        assertEquals(listOf(2L, 4L), drain(byCollection))

        val byArray = PriorityBlockingArrayQueueLong(listOf(1L, 2L, 3L, 4L))
    ...
    fun cloneIsIndependentAndEqual() {
        val original = PriorityBlockingArrayQueueLong(100, reverseComparator)
    ...
    fun equalsConsidersComparatorAndContent() {
        val a = PriorityBlockingArrayQueueLong(listOf(1L, 2L, 3L))
        val b = PriorityBlockingArrayQueueLong(listOf(1L, 2L, 3L))
        assertEquals(true, a == b)
        assertEquals(a.hashCode(), b.hashCode())

        val differentComparator = PriorityBlockingArrayQueueLong(listOf(1L, 2L, 3L), reverseComparator)
        assertEquals(false, a == differentComparator)

        val differentContent = PriorityBlockingArrayQueueLong(listOf(1L, 2L, 99L))
    ...
    fun toStringContainsSizeAndTop() {
        val q = PriorityBlockingArrayQueueLong(listOf(5L, 3L, 8L))
        val s = q.toString()
    ...
    fun forEachVisitsAllElements() {
        val q = PriorityBlockingArrayQueueLong(listOf(1L, 2L, 3L, 4L))
        var sum = 0L
        q.forEach { sum += it }
    ...
    fun spliteratorTryAdvanceAndSplitAndCharacteristics() {
        ...
        val tiny = PriorityBlockingArrayQueueLong(listOf(1L)).spliterator()
    ...
    fun streamAndParallelStream() {
        ...
        assertEquals(10L, PriorityBlockingArrayQueueLong(listOf(1L, 2L, 3L, 4L)).parallelStream().sum())
    }
}
