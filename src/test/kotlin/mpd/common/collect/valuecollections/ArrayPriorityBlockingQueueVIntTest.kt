...
class ArrayPriorityBlockingQueueVIntTest {
    ...
    fun constructors() {
        ...
        val wrapped = ArrayPriorityBlockingQueueVInt<QVIntTestClass>(ArrayPriorityBlockingQueueInt(listOf(3, 1, 2)))
    ...
    fun typedAddAndOffer() = with (QVIntTestClass) {
        val q = ArrayPriorityBlockingQueueVInt<QVIntTestClass>()
        assertEquals(true, q.add(QVIntTestClass(5)))
        assertEquals(true, q.offer(QVIntTestClass(3)))
        assertEquals(2, q.size)
    ...
    fun containsAndRemoveTyped() = with (QVIntTestClass) {
        val q = simpleQueue()
        assertEquals(true, q.contains(QVIntTestClass(200)))
        assertEquals(false, q.contains(QVIntTestClass(999)))
        assertEquals(true, q.remove(QVIntTestClass(200)))
        assertEquals(false, q.contains(QVIntTestClass(200)))
        assertEquals(2, q.size)
        assertEquals(false, q.remove(QVIntTestClass(999)))
    }
    ...
    fun removeAllPredicate() = with (QVIntTestClass) {
        ...
        assertEquals(true, q.contains(QVIntTestClass(100)))
    }
    ...
    fun priorityOrderingWithCustomComparator() = with (QVIntTestClass) {
        val maxHeap = ArrayPriorityBlockingQueueVInt<QVIntTestClass>(ArrayPriorityBlockingQueueInt(comparator = reverseComparator))
