...
class CollectionVLongTest {
    ...
    fun plusAndMinus() = with (VLongTestClass) {
        ...
        assertEquals(vLongListOf(VLongTestClass(1), VLongTestClass(2), VLongTestClass(3)), (array + VLongTestClass(3)))
    ...
    fun flatMapVariants() {
        context (VLongTestClass, MyIntTestClass) {
            val array = vLongListOf(VLongTestClass(1), VLongTestClass(2))
            val flatVLong: ArrayListVLong<VLongTestClass> = array.flatMap(fun(e: VLongTestClass): CollectionVLong<VLongTestClass> = vLongListOf(e, VLongTestClass(e.value * 10)))
            assertEquals(listOf(1L, 10L, 2L, 20L), flatVLong.mapGeneric { it.value })
            val flatVLongTo: ArrayListVLong<VLongTestClass> = array.flatMapTo(ArrayListVLong<VLongTestClass>()) { e: VLongTestClass -> vLongListOf(e) }
            assertEquals(2, flatVLongTo.size)
            val flatIndexedVLong: ArrayListVLong<VLongTestClass> = array.flatMapIndexed(fun(i: Int, e: VLongTestClass): CollectionVLong<VLongTestClass> = vLongListOf(VLongTestClass(i.toLong()), e))
            assertEquals(4, flatIndexedVLong.size)
            val flatIndexedVLongTo: ArrayListVLong<VLongTestClass> = array.flatMapIndexedTo(ArrayListVLong<VLongTestClass>()) { i: Int, e: VLongTestClass -> vLongListOf(e) }
            assertEquals(2, flatIndexedVLongTo.size)
            val flatVInt: ArrayListVInt<MyIntTestClass> = array.flatMap(fun(e: VLongTestClass): CollectionVInt<MyIntTestClass> = vIntListOf(MyIntTestClass(e.value.toInt())))
            assertEquals(listOf(1, 2), flatVInt.mapGeneric { it.value })
            val flatVIntTo: ArrayListVInt<MyIntTestClass> = array.flatMapTo(ArrayListVInt<MyIntTestClass>()) { e: VLongTestClass -> vIntListOf(MyIntTestClass(e.value.toInt())) }
            assertEquals(2, flatVIntTo.size)
            val flatIndexedVInt: ArrayListVInt<MyIntTestClass> = array.flatMapIndexed(fun(i: Int, e: VLongTestClass): CollectionVInt<MyIntTestClass> = vIntListOf(MyIntTestClass((i + e.value).toInt())))
            assertEquals(2, flatIndexedVInt.size)
            val flatIndexedVIntTo: ArrayListVInt<MyIntTestClass> = array.flatMapIndexedTo(ArrayListVInt<MyIntTestClass>()) { i: Int, e: VLongTestClass -> vIntListOf(MyIntTestClass(e.value.toInt())) }
    ...
    */
    ...
    fun plusMinusElementVariants() = with (VLongTestClass) {
        val array = vLongListOf(VLongTestClass(1), VLongTestClass(2))
        assertEquals(vLongListOf(VLongTestClass(1), VLongTestClass(2), VLongTestClass(3)), array.plusElement(VLongTestClass(3)))
    ...
    fun runningReduceVariants() = with (VLongTestClass) {
        val array = vLongListOf(VLongTestClass(1), VLongTestClass(2), VLongTestClass(3))
        assertEquals(vLongListOf(VLongTestClass(3), VLongTestClass(6)), array.runningReduceVLong { acc, e -> VLongTestClass(acc.value + e.value) })
        assertEquals(vLongListOf(VLongTestClass(3), VLongTestClass(6)), array.runningReduceVLongIndexed { _, acc, e -> VLongTestClass(acc.value + e.value) })
        ...
        context (longAdapterForVLongTestClass) {
            assertEquals(vIntListOf(VLongTestClass(3), VLongTestClass(6)), array.runningReduceVInt { acc, e -> VLongTestClass(acc.value + e.value) })
            assertEquals(vIntListOf(VLongTestClass(3), VLongTestClass(6)), array.runningReduceVIntIndexed { _, acc, e -> VLongTestClass(acc.value + e.value) })
        }
    }
