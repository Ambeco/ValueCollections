...
class MutableCollectionVLongTest {
    ...
    private fun simpleSet(): ArraySetVLong<MutColLongTestClass> = with (MutColLongTestClass) {
        ArraySetVLong<MutColLongTestClass>(20).also { it += MutColLongTestClass(100); it += MutColLongTestClass(200); it += MutColLongTestClass(300) }
    }
    ...
    fun plusAssignVariants() = with (MutColLongTestClass) {
        val set = ArraySetVLong<MutColLongTestClass>(10)
        set += MutColLongTestClass(1)
        assertEquals(1, set.size)
        set += arrayOf(MutColLongTestClass(2), MutColLongTestClass(3))
        assertEquals(3, set.size)
        set += listOf(MutColLongTestClass(4))
        assertEquals(4, set.size)
        set += listOf(MutColLongTestClass(5)).asIterable()
    ...
    fun removeAllVariants() = with (MutColLongTestClass) {
        ...
        assertEquals(true, byList.removeAll(vLongListOf(MutColLongTestClass(1), MutColLongTestClass(2))))
    ...
    fun minusAssignVariants() = with (MutColLongTestClass) {
        ...
        single -= MutColLongTestClass(100)
        assertEquals(2, single.size)

        val byList = simpleSet()
        byList -= vLongListOf(MutColLongTestClass(100), MutColLongTestClass(200))
        assertEquals(1, byList.size)

        val byArray = simpleSet()
        byArray -= arrayOf(MutColLongTestClass(100))
        assertEquals(2, byArray.size)

        val byCollection = simpleSet()
        byCollection -= listOf(MutColLongTestClass(100))
        assertEquals(2, byCollection.size)

        val byIterable = simpleSet()
        byIterable -= listOf(MutColLongTestClass(100)).asIterable()
    ...
    fun asIterableAndAsModifiableIterableOnIndexedBacking() = with (MutColLongTestClass) {
        ...
        val list: MutableCollectionVLong<MutColLongTestClass> = ArrayListVLong<MutColLongTestClass>().also { it += MutColLongTestClass(1); it += MutColLongTestClass(2) }
